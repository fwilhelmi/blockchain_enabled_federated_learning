...
root_folder_iid = 'Outputs/output_tensorflow/results_emnist/num_classes_10_cnn';
root_folder_noniid = 'Outputs/output_tensorflow/results_emnist/num_classes_3_cnn';
...
for k = 1 : length(users)      
    ...
    for p = 1 : length(partitions)
        % IID
        file_path = [root_folder_iid '/CNN_eval_accuracy_K' ...
        ...
        fclose(fid);        
        % Non-IID
        file_path = [root_folder_noniid '/CNN_eval_accuracy_K' ...
            num2str(users(k)) '_' num2str(partitions(p)) '.txt'];
        lgd{k} = strcat('K=',num2str(users(k))) ;
        fid = fopen(file_path);
        tline = fgetl(fid);
        ...
        results_noniid = [results_noniid B(1:200)];
