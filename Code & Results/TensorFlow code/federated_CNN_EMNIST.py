...
# NUM_CLIENTS = 200
SELECTED_MODEL = 3  # 1: First FNN, 2: Second FNN, 3: CNN
NUM_CLIENTS_TEST = 50
NUM_EPOCHS = 5
BATCH_SIZE = 20
SHUFFLE_BUFFER = 50
...
NUM_CLIENTS_PER_ROUND = [200] # Faltará 200 (1) IID during 500 iterations
...
for m in NUM_CLIENTS_PER_ROUND:
    ...
    for percentage in PERCENTAGES:
        ...
        final_model.save('model_CNN_EMNIST_' + str(m) + '_' + str(percentage) + '.h5')  # Save the model

        # SAVE THE RESULTS
        np.savetxt('CNN_train_loss_K' + str(m) + '_' + str(percentage) + '.txt',
                   np.reshape(train_loss, (1, NUM_ROUNDS_FL)))
        np.savetxt('CNN_test_loss_K' + str(m) + '_' + str(percentage) + '.txt',
                   np.reshape(test_loss, (1, NUM_ROUNDS_FL)))
        np.savetxt('CNN_test_accuracy_K' + str(m) + '_' + str(percentage) + '.txt',
                   np.reshape(test_accuracy, (1, NUM_ROUNDS_FL)))
        np.savetxt('CNN_eval_loss_K' + str(m) + '_' + str(percentage) + '.txt',
                   np.reshape(eval_loss, (1, NUM_ROUNDS_FL)))
        np.savetxt('CNN_eval_accuracy_K' + str(m) + '_' + str(percentage) + '.txt',
                   np.reshape(eval_accuracy, (1, NUM_ROUNDS_FL)))
        np.savetxt('CNN_iteration_time_K' + str(m) + '_' + str(percentage) + '.txt',
